...
function HomePage() {
  ...
  const onSubmit = (data) => {
    if (Object.keys(errors).length > 0) {
      showErrorModal("This is required");
    } else {
      console.log(data);
    }
  };
  ...
  return (
    <div className="relative flex items-center justify-center min-h-screen">
      ...
      <div className="max-w-3xl flex flex-col items-center justify-between mb-44  space-y-36    w-full p-4">
        ...
        <form
          className="relative flex items-center justify-end  md:w-1/2 w-full"
          onSubmit={handleSubmit(onSubmit)}
        >
          <input
            type="text"
            className="w-full p-3 md:px-9 md:py-4 rounded-2xl active:outline-none text-left  "
            placeholder="search your favorite movie..."
            {...register("searchinput", {
              required: "just type a film name",
            })}
          />
          <svg
          ...
          {errors.searchinput &&
            showErrorModal("Please type the name of a film you want to watch.")}
        </form>
      </div>
    </div>
  );
